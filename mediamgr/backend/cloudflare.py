...
CF_HOST = os.environ.get("CF_HOST", "10.99.0.50")
CF_USER = os.environ.get("CF_USER", "root")
CF_KEY  = os.environ.get("CF_KEY", "/root/.ssh/cf_key")
...
def _conn_opts() -> dict:
    ...
    if os.path.exists(CF_KEY):
        opts["client_keys"] = [CF_KEY]
    return opts
    ...
    async with _conn_lock:
        if _conn is None:
            _conn = await asyncio.wait_for(
                asyncssh.connect(CF_HOST, **_conn_opts()),
                timeout=10,
            )
    return _conn
...
async def list_tunnels() -> list[dict]:
    ...
    if rc != 0:
        return [{"error": err or "SSH failed", "name": "error", "running": False}]
    ...
    # Batch status check in one command
    if not services:
        return []
